        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-6">
          {filteredLetters.map((letter) => (
            <Link
              key={letter.id}
              to={`/trace/${letter.id}`}
              className="aspect-square bg-white rounded-3xl shadow-[0_8px_0_rgb(0,0,0,0.1)] hover:shadow-[0_4px_0_rgb(0,0,0,0.1)] hover:translate-y-1 transition-all duration-150 flex flex-col items-center justify-center border-2 border-indigo-100 relative overflow-hidden group"
            >
              <span 
                className="text-6xl font-bold font-fredoka mb-2 group-hover:scale-110 transition-transform" 
              ...
              {letter.word && (
                <div className="flex flex-col items-center">
                  <span className="text-2xl mb-1 filter drop-shadow-sm">{letter.emoji}</span>
                  <span className="text-xs font-bold text-gray-400 uppercase tracking-widest">{letter.word}</span>
                </div>
              )}
            </Link>
          ))}
